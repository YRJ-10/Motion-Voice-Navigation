chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'VOICE_COMMAND') {
    ...
    chrome.tabs.query({active: true}, function(tabs) {
        let targetTabs = tabs.filter(t => !(t.url && (t.url.startsWith('chrome://') || t.url.includes('dashboard.html'))));
        if (targetTabs.length === 0) return;
        
        targetTabs.forEach(activeTab => {
            if (cmd === 'tutup_tab') {
                chrome.tabs.remove(activeTab.id);
                return;
            } else if (cmd === 'geser_kanan' || cmd === 'geser_kiri') {
                chrome.tabs.query({windowId: activeTab.windowId}, (allTabs) => {
